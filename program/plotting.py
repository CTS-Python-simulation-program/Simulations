...
class Plot:
    ...
    def plot_shapes(self):
        ...
        rectangle = patches.Rectangle((0, 0), self.boxLen, self.boxWidth, linewidth=1, edgecolor='blue', facecolor='none')
        ax.add_patch(rectangle)
        circle = patches.Circle(self.circleCenterCoord, self.radius, linewidth=1, edgecolor='red', facecolor='none')
        ax.add_patch(circle)

        square_side = self.radius
        square = patches.Rectangle(
            (self.squareCenterCoord[0] - square_side / 2, self.squareCenterCoord[1] - square_side / 2),
